...
export default async function Customers({ searchParams }: { readonly searchParams: Promise<{ q?: string }> }) {
  ...
  return <AppShell><section className="section">
    <div className="toolbar"><div><div className="eyebrow">Clientes</div><h1>{normalizedSearch ? "Resultados de búsqueda" : "Cartera de clientes"}</h1><p className="muted">Consulta clientes, solicitudes, créditos y pagos desde un solo lugar.</p></div></div>
