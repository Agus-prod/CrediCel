-- Customers with an activated credit can remain visible through their branch
-- application even when a legacy seed did not create a customer assignment.
...
begin
  ...
  into v_branch_id
  from (
    select assignment.branch_id, 0 as source_priority
    ...
    union all
    select application.branch_id, 1 as source_priority
    from public.credit_applications as application
    where application.organization_id = v_organization_id
